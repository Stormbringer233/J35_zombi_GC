...
impl IWidgetBuilder for ButtonBuilder {
    ...
    fn caption(mut self, caption: &str) -> Self {
        ...
        if self.dimensions.length() == 0.0 {
            // no dimensions was provided before calling this function
...
impl Button {
    ...
    fn draw_debug(&self, m_state: &MouseStates) {
        ...
        draw_text(
            format!("mouse position : {:?}", m_state.position).as_str(),
            10.0,
            20.0,
            20.0,
            BLACK,
...
impl MacroDraw for Button {
    fn draw_flat(&self, theme: (FullComposite, FullComposite, FullComposite, FullComposite)) {
        ...
        // background
        draw_rectangle(x, y, w, h, Color::from_rgba(bg.0, bg.1, bg.2, bg.3));
        //liner
        draw_rectangle_liner(
            x,
            y,
            w,
            h,
            Color::from_rgba(lt.0, lt.1, lt.2, lt.3),
            Color::from_rgba(lb.0, lb.1, lb.2, lb.3),
        );
        // text
