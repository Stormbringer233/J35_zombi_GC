...
fn render_popup(x: f32, y: f32, datas: (&String, u32, u8, f32, f32)) {
    ...
    draw_text(&format!("Population : {} millions",datas.1 as f32/1000.), x_pop + 5.0, y_pop + 2.0*fs, fs, BLACK);
    draw_text(&format!("Number of districts : {}",datas.2), x_pop + 5.0, y_pop + 3.0* fs, fs, BLACK);
    draw_text(&format!("Secutity rate : {:.2} %",datas.3*100.), x_pop + 5.0, y_pop + 4.0*fs, fs, BLACK);
    draw_text(&format!("Zombi rate : {:.2} %",datas.4*100.), x_pop + 5.0, y_pop + 5.0*fs, fs, BLACK);
}
